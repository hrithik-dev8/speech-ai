...
async def evaluate(
    ...
    if not all(os.path.exists(p) for p in [pdf_path, audio_path]):
        logger.error(f"File not found: pdf_path exists? {os.path.exists(pdf_path)}, audio_path exists? {os.path.exists(audio_path)}")
        raise HTTPException(400, "Required files not found")
    
    try:
        transcript = transcribe_audio(audio_path) or ""
        return EvaluationResponse(
            delivery=evaluate_delivery(audio_path),
            content=evaluate_content(pdf_path, transcript),
